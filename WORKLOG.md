...
## ▶ 다음 세션 시작 체크리스트 (2026-08-28 세션 마감 기준)

**배포 상태**: `origin/master = 291e962` — 퇴원 폼 강화 1·2단계 배포 완료(DDL 2건 운영 적용 확인).
...
1. **[사람·즉시]** **휴원 4건 화면 전환** — 신윤후(7월)·이하율(7월)·이은겸(8월), 그리고 **김범찬은 7월 행만**(8월 행은 실제 퇴원이므로 그대로 둔다). 2026-08-28 실측 기준 **아직 0건 전환**(DB 168행 전부 `status='withdrawn'`). 전환하면 대시보드·월간 반성·강사 러닝 숫자가 **정확히 4 줄어야** 정상이고, 목록 행 수는 그대로여야 한다.
2. **[약속·9월 말]** **퇴원 재측정**. 기준선은 진단 보고서(https://claude.ai/code/artifact/d85f51ed-26d1-44ac-adca-3cfebac9a584). 비교 지표 넷 — 9월 퇴원 건수 / 회고 작성률(현재 **2%**) / 사유·기간 미입력(신규분 **0 유지 여부**) / 압박·남기기 반발형 재발 여부.
3. **[3단계 후보]** **퇴원 기록 ↔ 학생 명부 연결(`student_id`)** — 보고서 8장 '가장 값진 투자'. 완성하면 수능반 과목 중복(41행=36명)이 구조적으로 해소되고, "입회 성향분석이 퇴원을 예측했나" 분석이 열린다.
4. **[확인]** **영어 퇴원 기록 0건** — 168건이 전부 수학이다. 기록 누락인지 실제로 없는 건지 이번 주에 가른다.
5. **[보류 기록]** `src/lib/withdrawal-insight/events.ts:92` 재원기간 계산의 `enrollment_end` fallback — 실영향이 미미해 **의도적으로 보류**. `src/lib/chat-tools.ts` 쓰기 화이트리스트에 `status`·`notice_date` **미포함** — AI가 못 쓰게 둔 안전 기본값이다(둘 다 "빠뜨린 것"이 아니다).
6. **[기존 잔여 유지]** 야간 토글 **라이브 전환 육안 1건**(브라우저에 표시되지 않는 창에서 잰 측정은 오탐 — 이번 세션에서 같은 함정을 다시 확인했다) / 알림톡 **실발송 스모크 보류**(진단은 읽기 전용 `scripts/check-alimtalk-setup.mjs`) / `.nk-pf__*` 위젯 CSS **업무보고 원본으로 역이식 후보** / 병합 완료된 로컬 `feature/*` 브랜치 **10개 정리**(원격엔 3개) / Vercel `GEMINI_MODEL` 값 확인.
...
## 2026-08-26~28 — 퇴원 진단·문서 정합·퇴원 폼 강화 1·2단계

운영 퇴원 데이터를 전건 읽어 진단하고, 거기서 드러난 **기록 품질 문제를 폼에서 막도록** 두 단계로 손봤다. 진단이 먼저고 수리가 나중이다.

| 커밋 | 내용 |
|---|---|
| `6494e82` | 문서 정합 1차 — README·CLAUDE.md 를 현재 코드 기준으로 정정(불일치 12건) |
| `9cf7d89` | 문서 정합 2차 — 전문 재훑기로 잔여 13건(폰트·기능표·테이블 25개·구조도·환경변수) |
| `cd78191` | 폼 강화 1단계 — 퇴원일=마지막 등원일 확정, 인지일 분리, 필수 입력·날짜 정규화 |
| `291e962` | 폼 강화 2단계 — 휴원·복귀 상태 분리, 회고 7일 리마인더, AI 비서 매핑 교정 |

### 문서 정합 2커밋 (`6494e82`+`9cf7d89`)

README·CLAUDE.md 의 코드 불일치 **총 25건**을 정정했다. 1차에서 12건을 고쳤는데, 전문을 다시 훑자 **13건이 더 나왔다**.

**교훈 — 발견분만 고치면 문서 안에 새 모순이 생긴다.** 한 곳의 숫자를 고치고 같은 숫자를 인용한 다른 절을 놔두면 문서가 자기와 싸운다. 앞으로 문서 정정 위임에는 **"전문 훑기 + 모순 잔존 grep 검사"를 검증 단계로 명시**할 것.

### 7·8월 퇴원 진단 보고서

운영 DB **41건(실인원 36명)** 전건 + 서술 전문을 읽어 만든 진단(https://claude.ai/code/artifact/d85f51ed-26d1-44ac-adca-3cfebac9a584). 강사 공유용 PPT 16장(실화면 3장 포함)까지 전달했다.

- **7월 28건 = 기말 정산 이탈.** 시험이 끝나는 시점에 몰린다.
- **조용한 성실층이 빠진다** — 태도 중상 이상 **33명**, 출석 상 **24명**. 문제아가 나가는 게 아니다.
- **압박·남기기 반발형 6명**(그중 고2가 4명).
- **기록 자체가 비어 있다** — 회고 작성률 **2%**, 기록 공백 **83%**.
- **41행 = 36명** — 수능반 과목 중복 5행 + **휴원 혼입 4건**.
- **영어 퇴원 기록 0건** — 전건 수학(체크리스트 4번으로 이월).

### 폼 강화 1단계 (`cd78191`)

- **퇴원일 = 마지막 등원일**로 확정. 학원이 퇴원 사실을 알게 된 날은 **별도 칸 `notice_date`** 로 분리(DDL 적용 + 백필 **162/168** 실측). 그전까지 폼의 '퇴원인지일' 라벨이 실제로는 `enrollment_end` 에 저장되던 어긋남을 해소했다. `enrollment_end` 는 **deprecated·보존**(지우지 않는다).
- **필수 4칸** — 퇴원일·사유 분류·최종 상담일·상담 요약(30자 이상). 상담 일시는 자유 텍스트에서 **달력 입력**으로 교체.
- **붙여넣기 날짜 정규화** `src/lib/withdrawal-dates.ts` — `2026.01.15`→`2026-01-15`, `2026/1/5`→`2026-01-05`. **월까지만 있는 값은 채우지 않고 비워 둔다**(운영에 `"2026.01"` 30건이 이 경로로 들어와 있었다).
- **재원기간 자동 계산**(등록일·퇴원일에서 floor 개월). 손으로 고친 값은 다음 날짜 변경 전까지 유지.

**⚠ zod 4 함정** — `z.string().min(1, "메시지")` 만 쓰면 값이 **아예 안 넘어왔을 때**(undefined) 커스텀 메시지가 아니라 영문 기본 메시지(`Invalid input: expected string...`)가 사용자에게 나간다. 폼이 빈 값을 FormData 에서 빼고 보내므로 이게 실제로 터진다. 필수 항목은 **`z.string({ error: "메시지" })` 로 타입 에러 메시지까지 고정**해야 한다.

### 폼 강화 2단계 (`291e962`)

- **`status`(withdrawn/paused/returned) + `returned_at`** 추가(DDL 적용 확인). 기본값이 `'withdrawn'` 이라 적용 직후 화면 숫자는 변하지 않는다.
- **집계 규칙 — 퇴원 통계는 `withdrawn` 만 센다.** 판정은 `src/lib/withdrawal-status.ts` 순수 함수로 모으고, **필터는 각 클라이언트의 통계 계산 직전 한 지점**에만 건다(목록 화면은 휴원·복귀까지 전체를 보여야 하므로 서버에서 거르지 않는다).
- **휴원**은 예상 복귀 시기 필수. 자유 텍스트를 유지하되(`"9월 초"` 같은 서술형이 기존 데이터에 있다) **ISO 날짜로 적힌 경우에만 경과 뱃지**를 띄운다 — 서술형을 억지로 추측하지 않는다.
- **회고 D+7 리마인더**(앱 내 뱃지만, 알림톡 없음). 퇴원일+7일이 지났는데 회고가 미완이면 빨간 `회고 D+n`.
- 헤더 **"총 N명" → "퇴원 N명"**. 전환 전에는 숫자가 그대로여야 하므로 **`status` 미지정/`withdrawn` 배열은 필터 전후가 같다**를 테스트로 못 박았다.
- `src/app/api/chat/route.ts` — AI 비서에 넘기던 `퇴원일: w.enrollment_end` 를 **`withdrawal_date` 로 교정**하고 `상태` 를 추가. 쓰기 화이트리스트에는 넣지 않았다(위 체크리스트 5번).

### 검증

vitest **533 → 596**(신규 63) · tsc 0 error · lint 신규 0(기존 warning 29건 유지) · `next build` 성공. **각 단계마다 브레인이 독립적으로 재실행해 확인**했고, DDL 은 사용자가 SQL Editor 에서 적용한 뒤 REST 읽기로 실측 검증했다(백필 162/168, 168행 전부 `status='withdrawn'`).

### 실화면 촬영 방법 (다음에 또 필요할 것)

격리 worktree + dev 테스트 로그인 + playwright 로, **저장 없이** 실화면만 찍었다. worktree 경로는 **반드시 짧게** 잡을 것 — 긴 경로에서 Turbopack 이 Windows max path 를 넘겨 죽고, `node_modules` 를 정션으로 잇는 우회도 거부당한다(실설치해야 한다).
