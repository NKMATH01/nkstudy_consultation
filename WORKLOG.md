...
**배포 상태**: origin/master = `3351b1b` (Vercel 자동배포). 설문 V2 + 단일 보고서(7차까지) + 코럴 라이트 대시보드 스킨 전부 운영 반영.

**보고서 현재 형식 (7차)**: 카톡 전송용 모바일 문서(660px), 구조 = ①종합 분석(리드 총평+전영역 상세총평 3~4문단+영역별 한눈에) ②강점(점수 배지) ③약점(점수 근거+NK의 도움) ④항목별 분석(레이더+항목별 점수·밴드·3문장 해설) ⑤과목 이야기 ⑥NK 지도 계획 ⑦읽는 안내. 호칭은 "OO 학생"(AI에 이름 미전송, 서버 치환), 본문에 점수 수치 인용, 서술 간결(30% 축소). 상담자 화면·PDF·공유 링크 모두 동일 단일 보고서.

**오늘 학생 6명(강현찬·방준혁·양우준·김지민·서유진·김예찬)** 전원 7차 형식으로 재분석 완료(2026-07-12). 기존 V1 학생 데이터 불변. 신규 제출은 자동으로 최신 형식.

1. **[확인]** 사용자 보고서 실물 검토 → 추가 다듬기 피드백 가능성 (부분 스크린샷 방식)
2. **[보류]** 코럴 스킨 후속: 페이지 내부 콘텐츠 요소(월별 차트 네이비 막대·상담현황 툴바 등)까지 코럴 톤 통일 여부 — 사용자 결정 대기
3. **[보류]** SOLAPI 키 4종·알림톡 템플릿 등록(카카오 승인) — 등록 전 발송 불가
4. **[정리 후보]** /dev-report-preview·/design-preview 임시 라우트(프로덕션 가드 있음), CounselorReport 미사용 코드(복원 대비 보존), 사이드바 섹터 순서 변경 건(사용자 중지, 코럴 스킨으로 대체된 상태)
5. **[참고]** 분석 실행은 Gemini 20~60초 소요 — "안 된다" 문의 시 대기 안내. 재분석은 로컬 dev(운영 DB)에서 Playwright로 실행하는 패턴 검증됨.
