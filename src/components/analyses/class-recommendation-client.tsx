...
export function ClassRecommendationSection({ analysisId, studentName, studentGrade, initialTestScore }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-nk-line-soft bg-nk-surface p-5 shadow-sm">
      <div className="mb-4 flex items-center gap-2.5">
        <span
          className="flex h-9 w-9 items-center justify-center rounded-lg text-nk-navy-ink"
          style={{ background: "linear-gradient(135deg, var(--primary), var(--primary-soft))" }}
        >
          <GraduationCap className="h-4.5 w-4.5" />
        </span>
        <div>
          <h2 className="text-sm font-extrabold text-nk-ink">반 배정 추천</h2>
          <p className="text-[11px] font-semibold text-nk-ink-hint">
      ...
      <div className="mb-4 flex flex-wrap items-end gap-3">
        <label className="space-y-1">
          <span className="text-[11px] font-bold text-nk-ink-sub">테스트 점수 (0~100)</span>
          <Input
            value={testScore}
        ...
        <label className="space-y-1">
          <span className="text-[11px] font-bold text-nk-ink-sub">테스트 날짜</span>
          <Input type="date" value={testDate} onChange={(e) => setTestDate(e.target.value)} className="h-9" />
        </label>
        <label className="min-w-[200px] flex-1 space-y-1">
          <span className="text-[11px] font-bold text-nk-ink-sub">현재 진도 (선행 정도)</span>
          <Input
      ...
      {result && (
        <div className="space-y-3">
          <p className="text-xs font-semibold text-nk-ink-sub">
            테스트 등급 <span className="font-black text-nk-ink">{result.test_level}</span>
            {currentProgress && (
              <>
                {" · "}학생 진도 <span className="font-black text-nk-ink">{currentProgress}</span>
              </>
            )}
            {" — 추천 순"}
          </p>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {result.recommendations.map((rec, idx) => (
              <div
                key={rec.class_id}
                className="rounded-xl border bg-nk-surface p-4"
                style={{
                  borderColor: idx === 0 ? "var(--accent-warm)" : "var(--border, rgb(var(--wr-line)))",
                  boxShadow: idx === 0 ? "0 4px 16px color-mix(in srgb, var(--accent-warm) 22%, transparent)" : undefined,
                }}
              >
                <div className="mb-2 flex items-start justify-between gap-2">
                  <div>
                    <p className="text-sm font-extrabold text-nk-ink">
                      {idx === 0 && "⭐ "}
                      {rec.class_name}
                    </p>
                    <p className="text-[11px] font-semibold text-nk-ink-hint">
                ...
                <div className="mb-2 flex flex-wrap gap-1">
                  {TRAITS.map(({ key, label }) =>
                    rec[key] ? (
                      <span
                        key={key}
                        className="rounded bg-nk-sunken px-1.5 py-0.5 text-[10px] font-bold text-nk-ink-sub"
                      >
                ...
                {rec.main_textbook && (
                  <p className="mb-2 flex items-center gap-1 text-[11px] font-semibold text-nk-ink-sub">
                ...
                <ul className="space-y-0.5">
                  {rec.match_reasons.map((reason, i) => (
                    <li key={i} className="text-[11px] leading-relaxed text-nk-ink-sub">
                      · {reason}
                    </li>
                  ))}
                </ul>
