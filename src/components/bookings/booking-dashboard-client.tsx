...
function formatHourLabel(h: number) {
  return isSaturdayCode(h) ? getSlotLabel(h).split(" ")[0] : getSlotLabel(h);
}
...
export function BookingDashboardClient({ initialBookings, initialBlocked, initialTotal }: Props) {
  ...
  const getNoticeText = (b: Booking) => {
    ...
    const isSatSlot = isSaturdayCode(b.booking_hour);
    const testTimeStr = isSatSlot ? SATURDAY_LABELS[b.booking_hour] : `${b.booking_hour}:00~${b.booking_hour + 1}:00`;
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {[
                    ...
                    { l: "테스트", v: `${b.booking_date} ${SATURDAY_LABELS[b.booking_hour] || `${b.booking_hour}:00~${b.booking_hour + 1}:00`}` },
      ...
      <Dialog open={!!showNotice} onOpenChange={() => setShowNotice(null)}>
        <DialogContent className="max-w-lg">
          ...
          {showNotice && (() => {
            ...
            const subjectLabel = BOOKING_SUBJECTS.find((s) => s.id === b.subject)?.label || "-";
            const consultLabel = b.consult_type === "inperson" ? "대면상담" : "유선상담";
            const isSatSlot2 = isSaturdayCode(b.booking_hour);
            const consultTime = b.consult_type === "inperson"
