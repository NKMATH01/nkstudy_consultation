    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-[90vw] w-[90vw] max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5 mr-6">
              <button
                ...
                disabled={pdfLoading}
                className="h-8 px-3 rounded-lg text-[11px] font-bold flex items-center gap-1.5 transition-all hover:shadow-sm bg-nk-progress-soft text-nk-progress hover:bg-nk-progress-soft disabled:opacity-50"
                title="PDF 다운로드"
              >
              ...
              {!isV2 && <button
                ...
                className="h-8 px-3 rounded-lg text-[11px] font-bold flex items-center gap-1.5 transition-all hover:shadow-sm bg-nk-warn-soft text-nk-warn hover:bg-nk-warn-soft disabled:opacity-50"
                title="카카오톡 공유"
              >
              ...
              {!isV2 && <button
                ...
                className="h-8 px-3 rounded-lg text-[11px] font-bold flex items-center gap-1.5 transition-all hover:shadow-sm bg-nk-sunken text-nk-ink hover:bg-nk-sunken"
                title="링크 복사"
              >
        ...
        {isV2 && (
          <>
            {analysisId && (
              <div className="mb-3 rounded-xl border border-nk-cat-3 bg-nk-cat-3-soft p-3">
                <a
                  href={`/analyses/${analysisId}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex h-7 items-center gap-1.5 rounded-lg bg-nk-cat-3 px-3 text-[11px] font-bold text-nk-navy-ink transition-colors hover:bg-nk-cat-3"
                >
                  <Link2 className="h-3.5 w-3.5" />
                  V2 분석 결과지 열기
                </a>
        ...
        <>
        {/* 기본 정보 */}
        <h4 className="mt-2 text-xs font-bold text-nk-ink">기본·배경 응답</h4>
        <div className="mt-2 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
          {[
            ...
            <div key={label} className="rounded-lg bg-nk-sunken p-2.5">
              ...
              <p className="text-xs font-semibold mt-0.5" style={{ color: value ? "rgb(var(--wr-ink))" : "rgb(var(--wr-line))" }}>
                {value || "-"}
              </p>
            </div>
          ))}
        ...
        {survey.prev_complaint && (
          <div className="p-2.5 rounded-lg mt-1" style={{ background: "rgb(var(--wr-status-warn-soft))", borderLeft: "3px solid rgb(var(--wr-status-warn))" }}>
            <span className="text-[10px] font-semibold text-nk-warn">기존 학원 아쉬운점</span>
            <p className="text-xs mt-0.5 text-nk-warn">{survey.prev_complaint}</p>
          </div>
        )}
        ...
        {!isV2 && (
        <>
        <div className="mt-3 p-3 rounded-xl bg-nk-sunken">
